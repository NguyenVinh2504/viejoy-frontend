import { Box, Button, IconButton, Menu, Stack, styled, useMediaQuery } from '@mui/material'
...
function TextFieldComment({ onSubmit }) {
  ...
  const setCurrentEmoji = (emoji) => {
    if (Array.from(formik.values.comment).length < 400) {
      formik.setFieldValue('comment', formik.values.comment + emoji.native)
    }
  }
  return (
      ...
      <Stack direction={'row'} spacing={2} alignItems={'flex-start'}>
        ...
        <Box sx={{ flex: 1 }}>
          ...
          <Stack
            ...
            justifyContent={pointDownMd ? 'flex-end' : 'space-between'}
          >
            {/* emoji */}
            {pointDownMd ? null : (
              <>
                ...
                >
                  <Picker data={data} onEmojiSelect={setCurrentEmoji} previewPosition='none' theme='dark' />
                </Menu>
              </>
            )}
            {/* <Stack
            ...
            <Stack direction={'row'} justifyContent={'flex-end'}>
              ...
              <CustomButton sx={{ ml: 2 }} type='submit' disabled={!formik.values.comment || !formik.isValid}>
                Bình luận
              </CustomButton>
            </Stack>
          </Stack>
