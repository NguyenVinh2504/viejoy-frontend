...
function TitleMovieDetail({ loading, dataDetail, genres, mediaType }) {
  ...
  const rating = {
    ...
    date: formatDate(dataDetail?.release_date || dataDetail?.first_air_date),
    genres: genres?.join(', '),
      ...
      <Stack direction={'row'} alignItems={'center'} justifyContent={'space-between'} spacing={2}>
        {!loading ? (
          <>
            ...
            <Box>
              <ButtonAddFavorite item={dataDetail} isFavorite={dataDetail.isFavorite} mediaType={mediaType} />
            </Box>
          </>
        ) : (
          <>
        ...
        <>
          ...
          >
            {Object.entries(rating).map(([key, value]) => {
              ...
              return (
                value && (
                  <Typography key={key} variant={pointDownSm ? 'subtitle2' : 'subtitle1'} component={'p'}>
                    {value}
                  </Typography>
                )
              )
          ...
          <Box sx={{ display: { xs: 'block', sm: 'none' } }}>
            ...
            >
              {Object.entries(rating).map(([key, value]) => {
                ...
                return (
                  value && (
                    <Typography key={key} component={'p'} variant={pointDownSm ? 'subtitle2' : 'subtitle1'}>
                      {value}
                    </Typography>
                  )
                )
