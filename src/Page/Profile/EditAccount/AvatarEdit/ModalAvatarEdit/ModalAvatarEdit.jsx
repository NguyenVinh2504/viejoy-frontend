...
function ModalContainer({ children, isEditing, menuSize, setMenuSize }) {
  ...
  useEffect(() => {
    if (containerRef.current instanceof HTMLElement && containerRef.current.firstElementChild) {
  ...
  const getDimensionValue = (size, defaultValue = 'auto') => {
    return typeof size === 'number' && !Number.isNaN(size) ? `${size}px` : defaultValue
  }
...
function ModalAvatarEdit({ open, handleClose }) {
  ...
  const handleUploadAvatar = useCallback(async () => {
    ...
    try {
      const compressedFile = await imageCompression(avatarFile, IMAGE_COMPRESSION_OPTIONS)
      ...
      if (err) {
        toast.update(toastId, {
          render: err?.data?.message ?? 'Cập nhật ảnh đại diện không thành công',
          type: 'error',
          isLoading: false,
          autoClose: 3000
        })
  ...
  const calcHeightWhenResize = debounce(() => {
    calcHeight(isEditing ? editElementRef.current : previewElementRef.current)
  }, 500)
  useEffect(() => {
  ...
  return (
    <Modal open={open} aria-labelledby='modal-modal-title' aria-describedby='modal-modal-description'>
      ...
      <Fade in={open} timeout={300}>
        ...
        >
          <ModalContainer isEditing={isEditing} menuSize={menuSize} setMenuSize={setMenuSize}>
            <CSSTransition
              nodeRef={previewElementRef}
              in={!isEditing}
              timeout={500}
              unmountOnExit
              onEnter={() => calcHeight(previewElementRef.current)}
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <IconButton color='neutral' onClick={() => handleClose()} disabled={isDisabled}>
                    <CloseIcon />
                  </IconButton>
                </Stack>
                {/* title */}
                ...
                <label htmlFor='avatar'>
                  ...
                  >
                    ...
                    />
                    <Avatar src={previewAvatar && previewAvatar.preview} alt={null} />
                    <Box
                      sx={{
                        borderRadius: '1000px',
                ...
                <Stack flexDirection={'row'} gap={2}>
                  ...
                  <Button disabled={isDisabled} variant='contained' color='secondary' onClick={handleUploadAvatar}>
                    Cập nhât
                  </Button>
                  <Button disabled={isDisabled} variant='contained' color='secondary' onClick={handleDeleteAvatar}>
                    Xóa
                  </Button>
                </Stack>
                {/* Button */}
                  ...
                  <Grid item xs={12} md={4} lg={3}>
                    <Paper variant='outlined' sx={{ p: 2 }}>
                      <List component='nav' aria-label='main mailbox folders' sx={{ py: 0 }}>
                        ...
                        >
                          <ArrowHorizontalIcon />
                          <ListItemText primary={'Lật ngang'} />
                        </ListItemButton>
                        <ListItemButton
                          sx={{
                            gap: 2
                  ...
                  <Grid item xs={12} md={8} lg={9}>
                    <Paper variant='outlined' sx={{ p: 2 }}>
                      <Cropper
                        ...
                        highlight={false}
                        aspectRatio={1 / 1}
                        src={previewAvatar && previewAvatar.preview}
                        ref={cropperRef}
                        viewMode={1}
                        guides={true}
