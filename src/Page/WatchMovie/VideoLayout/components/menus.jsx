  ...
  return (
    <Menu.Root>
      <Tooltip.Root>
        ...
        <Tooltip.Content className={tooltipStyles.tooltip} placement={'top'}>
          Cài đặt
        </Tooltip.Content>
      </Tooltip.Root>
    ...
    (value) => {
      try {
        setStorage((prev) => {
          ...
          if (JSON.stringify(valueToStore) === JSON.stringify(defaultValue)) {
  ...
  return (
    <Menu.Root>
      ...
      <Menu.Content className={styles.submenu}>
        <SettingsFontFamily resetTrigger={resetTrigger} setResetTrigger={setResetTrigger} />
        ...
        <button onClick={handleReset} className={`${styles.submenuButton} ${styles.resetButton}`}>
          Đặt lại
        </button>
      </Menu.Content>
    </Menu.Root>
...
const SettingsFontFamily = memo(function SettingsFontFamily({ resetTrigger }) {
  ...
  usePlayerStyle('--media-user-font-family', fontFamily.value, DEFAULT_SETTINGS.fontFamily.value)
  ...
  return (
    <Menu.Root>
      ...
      <Menu.Content className={styles.submenu}>
        <Menu.RadioGroup className={styles.radioGroup} value={fontFamily.label}>
...
const SettingsFontColor = memo(function SettingsFontColor({ resetTrigger }) {
  ...
  usePlayerStyle('--media-user-font-color', debouncedColor, DEFAULT_SETTINGS.fontColor)
    ...
    <Menu.Root>
      ...
      <Menu.Content className={styles.submenu}>
        <Stack direction={'row'} alignItems={'center'} justifyContent={'space-between'} p={2}>
          ...
          <input type='color' value={color} name='color' onChange={handleColorChange} />
        </Stack>
      </Menu.Content>
    </Menu.Root>
  )
...
const SettingsFontSize = memo(function SettingsFontSize({ resetTrigger }) {
  ...
  const parsedFontSize = useMemo(() => String(parseFloat(fontSize) / 100), [fontSize])
  const defaultParsedFontSize = useMemo(() => String(parseFloat(DEFAULT_SETTINGS.fontSize) / 100), [])

  usePlayerStyle('--media-user-font-size', parsedFontSize, defaultParsedFontSize)
      ...
      <Menu.Content className={styles.submenu}>
        <Menu.RadioGroup className={styles.radioGroup} value={fontSize}>
          {FONT_SIZE_OPTIONS.map((size) => (
            <Menu.Radio className={styles.radio} value={size} onSelect={() => handleSelect(size)} key={size}>
              <CheckIcon className={styles.radioIcon} />
...
const SettingsBackgroundColor = memo(function SettingsBackgroundColor({ resetTrigger }) {
  ...
  useReset(resetTrigger, DEFAULT_SETTINGS.backgroundColor, setBackgroundColor)
  ...
  useEffect(() => {
    updatePlayerStyle(player, '--media-user-text-bg', rgbaBg, 'rgba(0, 0, 0, var(--media-user-text-bg-opacity, 1))')
  }, [player, rgbaBg])
  ...
  return (
    <Menu.Root>
      ...
      <Menu.Content className={styles.submenu}>
        <Stack direction={'row'} alignItems={'center'} justifyContent={'space-between'} p={2}>
          <Typography variant='body2'>Color</Typography>
          <input type='color' value={backgroundColor} name='color' onChange={handleColorChange} />
        </Stack>
      </Menu.Content>
    </Menu.Root>
  )
...
const SettingsBackgroundOpacity = memo(function SettingsBackgroundOpacity({ resetTrigger }) {
  ...
  const parsedOpacity = useMemo(() => String(parseFloat(opacity) / 100), [opacity])
  const defaultParsedOpacity = useMemo(() => String(parseFloat(DEFAULT_SETTINGS.backgroundOpacity) / 100), [])

  usePlayerStyle('--media-user-text-bg-opacity', parsedOpacity, defaultParsedOpacity)
...
function CaptionsSubmenu() {
  ...
  return (
    <Menu.Root>
      <SubmenuButton label='Phụ đề' hint={hint} disabled={options.disabled} icon={ClosedCaptionsIcon} />
      <Menu.Content className={styles.submenu}>
...
export function SpeedSubmenu() {
  const options = usePlaybackRateOptions(),
    hint = options.selectedValue === '1' ? 'Bình thường' : options.selectedValue + 'x'
  return (
    <Menu.Root>
      <SubmenuButton label='Tốc độ' hint={hint} disabled={options.disabled} icon={SpeedIcon} />
      <Menu.Content className={styles.submenu}>
        <Menu.RadioGroup className={styles.radioGroup} value={options.selectedValue}>
          {options.map(({ label, value, select }) => (
            <Menu.Radio className={styles.radio} value={value} onSelect={select} key={value}>
