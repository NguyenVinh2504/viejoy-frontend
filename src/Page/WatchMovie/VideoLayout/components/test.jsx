...
function SettingsFontFamily() {
  ...
  useEffect(() => {
    updatePlayerStyle(player, '--media-user-font-family', fontFamily.value, FONT_FAMILY_OPTIONS[3].value)
...
function SettingsFontSize() {
  const FONT_SIZE_OPTIONS = useMemo(() => ['50%', '75%', '100%', '125%', '150%', '200%'], [])
  const [currentFontSize, setCurrentFontSize] = useState(FONT_SIZE_OPTIONS[2])
  const player = useMediaPlayer()

  useEffect(() => {
    updatePlayerStyle(player, '--media-user-font-size', parseFloat(currentFontSize) / 100, 1)
  }, [player, currentFontSize])
  return (
    <Menu.Root>
      <SubmenuButton label='Cỡ chữ' hint={currentFontSize} />
      <Menu.Content className={styles.submenu}>
        <Menu.RadioGroup className={styles.radioGroup} value={currentFontSize}>
          {FONT_SIZE_OPTIONS.map((fontSize) => (
            ...
            >
              <CheckIcon className={styles.radioIcon} />
              <span className='media-radio-label'>{fontSize}</span>
            </Menu.Radio>
          ))}
        </Menu.RadioGroup>
      </Menu.Content>
...
function SettingsBackgroundOpacity() {
  const BACKGROUND_OPACITY_OPTIONS = useMemo(() => ['0%', '25%', '50%', '75%', '100%'], [])
  const [backgroundOpacity, setBackgroundColor] = useState(
    BACKGROUND_OPACITY_OPTIONS[BACKGROUND_OPACITY_OPTIONS.length - 1]
  )
  const player = useMediaPlayer()
  useEffect(() => {
    updatePlayerStyle(player, '--media-user-text-bg-opacity', parseFloat(backgroundOpacity) / 100, 1)
  ...
  return (
    <Menu.Root>
      ...
      <Menu.Content className={styles.submenu}>
        <Menu.RadioGroup className={styles.radioGroup} value={backgroundOpacity}>
